...
class PatientList extends React.Component { 
    constructor(props) {
        ...
        this.state = {
            ...
            patients: [],
            guideline_patients: [],
            guideline_patients_bool: false,
            guideline_sort_title: 'Alle',
            patientsPerPage: 60,
            noPatients: true,
...
// }
  ...
  sort() {
      ...
      if(event.target.value === "mostComments") {
        patients.sort(function(a, b){ 
          var counter_a = 0
          JSON.parse(a.guidelines).map(guideline => (counter_a +=guideline.comments.length))
          var counter_b = 0
          JSON.parse(b.guidelines).map(guideline => (counter_b +=guideline.comments.length))
          if(counter_a < counter_b) { return 1; } 
          else { return -1;  }
         });
        this.setState({sort: event.target.value, patients: patients});
      }
      if(event.target.value === "all guidelines") {
        this.setState({guideline_patients: [], guideline_sort_title: 'Alle', guideline_patients_bool: false});
      }
      if(event.target.value !== "all guidelines") {
        var guideline_patients = []
        var title = ''
        patients.forEach(patient => {
          let bool = false //= JSON.parse(patient.guidelines).filter(guideline => guideline.uid == event.target.value)
          JSON.parse(patient.guidelines).forEach(guideline => {
            if(guideline.uid === event.target.value){
              bool = true
              title = guideline.title
            }
          })
          if(bool) {
            guideline_patients.push(patient)
          }
        })
        this.setState({guideline_patients: guideline_patients, guideline_patients_bool: true});
      }
  }
  ...
  makeMandit(uid){
    let patient = this.state.patientClicked
    let guidelines = JSON.parse(patient.guidelines)
    guidelines.forEach(guideline => {
      if(guideline.uid === uid) {
        guideline.mandat = true
      }
    })

    patient.guidelines = JSON.stringify(guidelines)
    fetch('/api/patient/comments', {
                  method: 'post',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ new_guidelines: JSON.stringify(guidelines), firstname: patient.firstname, lastname: patient.lastname, created_on: patient.created_on})
    })
    .then(this.setState({patientClicked: patient}))
  }
  ...
  render() {
    ...
    document.title = "Patients - Homecare"
        ...
        if(this.state.guideline_patients_bool){
          Patients = this.state.guideline_patients
        }
      ...
      //sort comments and votes of this patients. Most recent first
      var guideline_activity = []
      // if(this.state.loadPatientClicked) {
      //   JSON.parse(this.state.patientClicked.guidelines).forEach(guideline => {
      //     guideline.comments.forEach(comment => {
      //       comment.surveyorvote = "survey"
      //       comment.guideline = guideline.short_title
      //       guideline_activity.push(comment)
      //     })
      //     guideline.votes.forEach(vote => {
      //       vote.surveyorvote = "vote"
      //       vote.guideline = guideline.short_title
      //       guideline_activity.push(vote)
      //     })
      //   })
      //   if(guideline_activity.length > 0) 
      //     guideline_activity = guideline_activity.sort(function(a, b){ if(Number(a.created_on) < Number(b.created_on)) { return -1; } if(Number(a.created_on) > Number(b.created_on)) { return 1; } return 0; }).reverse()
      // }


      if (!this.state.profile && !this.state.edit) {
        document.title = "Patients - Übersicht"
        return (
            <div className="container-fluid">
...

              {
               ...
               <div className="col-5">
                  <div className="card userClickedCard">
                              ...
                              <div className="row userclicked_comments">
                               <div className="col-12" style={{color: 'white'}}>
                                <Tabs>
                                    ...
                                    <TabPanel>
                                      <div className="col-12 userclicked_comments_scroll" style={{paddingTop: '20px'}}>
