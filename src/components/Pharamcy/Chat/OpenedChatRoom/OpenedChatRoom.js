...
class OpenedChatRoom extends Component {
  ...
  render() {
      ...
      let partner_type = ''
      if(this.props.myType !== this.props.active_chatroom.toType) 
        partner_type = this.props.active_chatroom.toType
      else 
        partner_type = this.props.active_chatroom.fromType
        ...
        <div className="col-6" style={{paddingRight: '0', paddingLeft: '0'}}>
          <div className="row">
          	<div className="col-12 top_banner_chat vertical_center_parent" style={{width: '50%'}}>
	                <div className="vertical_center_child">
	                  {this.props.active_chatroom.fromType !== 'pharamcy' ? (<p style={{fontWeight: 'bold', textTransform: 'capitalize'}} >{partner_type}: {this.props.active_chatroom.from}</p>) : (<p style={{fontWeight: 'bold', textTransform: 'capitalize'}} >{partner_type}: {this.props.active_chatroom.to}</p>)}
	                  <p style={{marginTop: '-16px'}} >{this.props.active_chatroom.name}</p>
	                </div>
          	</div>

            <div className="col-12 chat_messages_full" style={{height: 'calc(100vh - 140px)'}}>

              {this.props.active_chatroom.messages.messages.map((message, index) => {
              	if(message.type === 'message') {
              		return(<ChatMessage key={index} message={message.message} name={message.name} my_id={this.props.my_id} my_type={'pharmacy'} patner_name_1={this.props.active_chatroom.from} patner_type_1={this.props.active_chatroom.fromType} patner_name_2={this.props.active_chatroom.to} fromType={message.fromType} read={message.read}  />)
              	} else {
